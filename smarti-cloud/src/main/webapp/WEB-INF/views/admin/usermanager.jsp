...
<div class="row">
        ...
        <ul class="list-group menu list-unstyled">
	        <c:forEach items="${users.items}" var="item" varStatus="loop">
	            <li class="list-group-item"><a href='<c:url value="/admin/usereditor.html?path=${item.path}"></c:url>'>${item.title}</a><a href="javascript:deleteUser('${item.path}')"><img title="点击删除" class="pull-right" src='<c:url value="/resources/images/cut.gif"></c:url>'></a>
	            </li>           
	        </c:forEach> 
        </ul>
        </nav>
