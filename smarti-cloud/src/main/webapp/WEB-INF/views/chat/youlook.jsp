...
<div class="row">
	...
	<c:if test="${folder.title != null}">
        ...
        <nav class="wb-sec col-md-3 col-md-pull-9" typeof="SiteNavigationElement" id="wb-sec" role="navigation">
        		<h3 class="wb-navcurr"><a href="/site/youlook.html"><span class="glyphicon glyphicon-th-list"></span> 通讯目录</a></h3>
<%-- 		<c:if test="${user.role =='Owner' || user.role == 'Administrator'}"> --%>
			<details>
...
<%-- 		</c:if> --%>
        <ul class="list-group menu list-unstyled">
	        <c:forEach items="${folders.items}" var="item" varStatus="loop">
	            <li class="list-group-item" id="${item.uid }"><a  href='<c:url value="/site/youlook.html?path=${item.path}"></c:url>'><span class="glyphicon glyphicon-user">${item.title}</span><span id="unread-${item.uid }" class="badge"></span></a>
