...
/*	    		  +'<div class="form-group">'
				  ...
				  +'</div>'	
				  +'<div class="form-group">'
				  +'<label for="size'+data.uid+'">地址&nbsp;'+data.position+'</label><input class="form-control" id="size'+data.uid+'" name="size" value="'+data.device+'" size="24" uid="'+data.uid+'" disabled/>'
				  +'</div>'	
				  +'<div class="form-group">'
				  +'<label for="lastModified'+data.uid+'">文件日期&nbsp;</label>'
				  +getDateString(data.originalDate)
				  +'</div>'			  
				  +'</details>'
	    		  +'</div>';	
	    	$("#top_insert").after(html);	
