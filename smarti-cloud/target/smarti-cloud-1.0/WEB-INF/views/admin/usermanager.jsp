...
<div class="row">
        ...
        <ul class="list-group menu list-unstyled">
	        <c:forEach items="${users.items}" var="item" varStatus="loop">
	            <li id="${item.uid }" class="list-group-item"><a href='<c:url value="/admin/usereditor.html?path=${item.path}"></c:url>'>${item.title}</a><a href="javascript:removeTag('${item.uid}')"><span class="glyphicon glyphicon-trash pull-right"></span></a>
	            </li>           
	        </c:forEach> 
        </ul>
        </nav>
