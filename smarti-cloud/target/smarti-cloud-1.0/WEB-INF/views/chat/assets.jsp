...
<div id="{uid}" class="col-md-4">
	<div class="checkbox">
		<input type="checkbox" class="checkbox" name="puid" value="{uid}"><a title="打开PDF" href="<c:url value="/site/viewpdf?uid={uid}"/>" target="_BLANK"><img title="点击选中" src='<c:url value="/resources/images/pdf.gif"></c:url>'></a>
		<a class="wb-lbx-edit" href="<c:url value='/site/{link}'></c:url>" target="_BLANK"><imgreplace id="img{uid}" src="<c:url value='/site/{icon}'></c:url>" class="img-responsive" draggable="true"></a>
	</div>
	<details>
		<summary><span class="glyphicon glyphicon-eye-open"></span>{title}</summary>
	</details>
</div>	 
</div>	   
...
<div class="row">
	...
	<div class="checkbox">
        <c:if test="${item.pdf}">
		<input type="checkbox" class="checkbox" name="puid" value="${item.uid }"><a title="<spring:message code="djn.open"/>PDF" href="<c:url value="viewpdf?uid=${item.uid}"/>" target="_BLANK"><span class="glyphicon glyphicon-open"></span>PDF</a>
		</c:if>
        <c:if test="${item.text}">
...
<%-- 		<a class="btn bnt-default" href="editImage.html?uid=${item.uid}">Edit</a> --%>
	...
	<details>
		...
		<a class="wb-lbx" title="<spring:message code="djn.delete"/>" href="<c:url value='deleteasset.html?path=${item.path}&redirect=/site/assets.html?path=${folder.path }'/>"><span class="glyphicon glyphicon-remove"></span><spring:message code="djn.delete"/>(${item.path })</a>
		<div class="form-group">
		...
		<div class="form-group">
		<label for="lastModified${item.uid }"><spring:message code="djn.doc_date"/>&nbsp;</label>
